...
fn load_import_patterns_from_mappings() -> Result<Vec<String>, String> {
    ...
    let mappings_path = rules_dir.join("javascript").join("mappings.json");
    ...
    for import_path in file.mappings.keys() {
        patterns.insert(format!("require(\"{import_path}\")"));
        patterns.insert(format!("require('{import_path}')"));
        patterns.insert(format!("import {import_path}"));
        patterns.insert(format!("import * from \"{import_path}\""));
        patterns.insert(format!("import * from '{import_path}'"));
...
pub struct JavaScriptCryptoFilter;

impl CryptoFileFilter for JavaScriptCryptoFilter {
        ...
        let patterns = get_crypto_import_patterns().map_err(|e| {
            FilterError::FileRead(format!(
                "Failed to load crypto import patterns from classifier-rules: {e}. \
                 Ensure classifier-rules/javascript/mappings.json exists and is valid."
            ))
        })?;
