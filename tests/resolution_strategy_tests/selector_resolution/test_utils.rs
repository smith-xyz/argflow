//! Shared test utilities for selector resolution tests
...
pub fn get_arg_expression(result: &ScanResult, arg_idx: usize) -> Option<String> {
    result
        .calls
        .first()
        .and_then(|c| c.arguments.get(arg_idx))
        .filter(|a| !a.expression.is_empty())
